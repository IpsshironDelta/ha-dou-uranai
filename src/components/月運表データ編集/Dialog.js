...
//　定数
////////////////////////////////////////////
const MonthUnnHyouData = "MonthUnHyouData"
...
export default function FormDialog(props) {
  ...
  const handleChange = (getText , getID , getType) => {
    ...
    getDocs(collection(firestore, MonthUnnHyouData)).then((querySnapshot)=>{
      querySnapshot.forEach((document) => {
        if(document.data().MonthBaiorizumNum == getID){
          const GetsuUnnBaiorizumRef = doc(firestore , MonthUnnHyouData , document.id)
          switch (getType){
            case "MonthBaiorizumText":
              console.log("★★" , document.data().MonthBaiorizumNum , getType)
              updateDoc(GetsuUnnBaiorizumRef, {
                MonthBaiorizumText     : getText,
