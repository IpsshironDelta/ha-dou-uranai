...
export default function FormDialog(props) {
  ...
  const handleChange = (getText , getID , getType) => {
    ...
    getDocs(collection(firestore, NennUnnHyouData)).then((querySnapshot)=>{
      querySnapshot.forEach((document) => {
        if(document.data().YearBaiorizumNum == getID){
          const NennUnnBaiorizumRef = doc(firestore , NennUnnHyouData , document.id)
          switch (getType){
            case "YearBaiorizumText":
              console.log("★★" , document.data().YearBaiorizumNum , getType)
              updateDoc(NennUnnBaiorizumRef, {
                YearBaiorizumText     : getText,
